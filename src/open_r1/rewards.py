...
model_path = "src/SFT_model"
tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)

#  todo 晚上检查 acc acc还是没计算好
...
def accuracy_reward2(completions, solution, **kwargs):     
    ...
    rewards = []
    for answer, ground_truth in zip(contents, solution):
        reward = 0
        avg_table_fail_penalty = 0
        avg_column_fail_penalty = 0
...
def my_len_reward(completions: list[Dict[str, str]], **kwargs) -> float:
    """Compute length-based rewards to discourage overthinking and promote token efficiency.
    ...
    for content in contents:
        reward = 0.0
        complete_token = tokenizer(content)
        complete_token_len = len(complete_token["input_ids"])
        if 0 < complete_token_len < 600:
            reward += 1.0
        elif 600 <= complete_token_len < 800:
            reward += 2.0
        elif 800 <= complete_token_len < 1200:
            reward += 0.0
        elif 1200 <= complete_token_len < 2000:
            reward -= 2.0
        elif 2000 <= complete_token_len:
            reward -= 4.0
        rewards.append(reward)
